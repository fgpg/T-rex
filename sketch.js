...
function setup(){
  ...
  gameOverTextObj = createSprite(300, 75);
...
function draw(){
  ...
  //start || restart
    ...
    text("Presiona R para iniciar, o reiniciar", 225, 100);
  }
...
function nubes()
{
  if (frames%60==0)
  {
    ...
    var nube = createSprite(600+rnd, 100+rnd*2, 75, 50);
    G_nubes.add(nube);
...
function cactusGen()
{
  ...
  {
    //nextcactusframes=15+Math.round(random(-3,3));
    var cx = 600+Math.round(random(-5, 5));
    var cscale = 0.6+random(-0.01, 0.01);
     var cactus = createSprite(cx, 170, 15, 50);
